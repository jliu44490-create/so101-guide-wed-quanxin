...
**接手前先读仓库根目录的 `HANDOFF.md`** —— 里面有中国版的整体思路、已完成、卡点、剩余事项和恢复步骤,读它即可无缝接上。

一句话:国际版 `lvjin.online` 一直正常;中国版认证已搭通并部署上线 `cn.lvjin.online`(香港),**但大陆直连太慢、暂停**,等定"换 CloudBase 云托管(上海)还是备案"的方向;社区评论云函数代码写完未部署。详见 `HANDOFF.md`。
